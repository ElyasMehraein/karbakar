...
export async function PUT(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const { businessID, basket } = body as { businessID: string; basket: BasketItem[] };
        ...
        const business = await BusinessModel.findById(businessID).populate("guild") as Business;
        ...
        if (basket.length === 0) {
            ...
            return NextResponse.json({ message: "Monthly commitment cleared as basket is empty" }, { status: 200 });
        }
        ...
        return NextResponse.json({ message: "Business successfully updated" }, { status: 201 });
