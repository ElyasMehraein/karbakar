...
export async function DELETE(req: Request): Promise<NextResponse> {
    try {
        ...
        const myBusiness: Business = await BusinessModel.findById(myBusinessID).populate("guild");
        if (!myBusiness) {
        ...
        // پیدا کردن اتحاد
        const union: Union = await UnionModel.findById(unionID).populate("members.member");
        if (!union) {
