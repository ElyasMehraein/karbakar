...
export async function GET(req, res) {
  ...
  try {
    ...
    connectToDB()
    const logedUser =
      tokenPayLoad && (await UserModel.findOne({ _id: tokenPayLoad.id }).populate('businesses'))
    ...
    if (logedUser) {
      ...
      requests = await RequestModel.find({}, 'title message guild acceptedBy').sort({
        createdAt: -1,
      })
      return Response.json(
        { message: 'get Requests for guests successfully', data: requests },
        { status: 200 }
      )
    }
  } catch (error) {
    console.error(`Error get reports`, error)
    Response.json({ message: `Error get reports`, error }, { status: 500 })
  }
}
