...
export async function GET(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        connectToDB()
        const logedUser = tokenPayLoad && await UserModel.findOne({ _id: tokenPayLoad.id }).populate("businesses") as User;
        if (!logedUser) {
            return NextResponse.json(
                { message: "no logged user" },
                { status: 403 })
        ...
        const requests = await RequestModel.find({ uniqCode }).populate("acceptedBy").sort({ createdAt: -1 }) as Request[];
