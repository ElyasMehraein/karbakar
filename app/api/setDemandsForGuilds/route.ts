...
export async function PUT(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        if (!loggedUser) {
            return NextResponse.json({ message: "Please log in first" }, { status: 404 });
        }
        if (!jobCategory) {
            return NextResponse.json({ message: "jobCategory is required." }, { status: 407 });
        }
        ...
        if (!selectedGuild) {
            return NextResponse.json({ message: "no guild selected" }, { status: 400 });
        }
        ...
        if (isItMoreThan30) {
            return NextResponse.json({ message: "you can not have more than 30 demands for guilds" }, { status: 422 });
        }
        ...
        if (!existingDemand) {
            ...
            return NextResponse.json({ message: "demand already exist" }, { status: 406 });
        }

        return NextResponse.json({ message: "Business successfully updated", data: guild._id }, { status: 201 });
