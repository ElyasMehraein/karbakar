...
export async function GET(req, { params }) {
  try {
    ...
    if (!token) {
      return new Response(JSON.stringify({ message: 'Authentication required' }), { status: 403 })
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ message: 'Invalid token', error: error.message }), {
        status: 401,
      })
    }
    ...
    // بازگرداندن گزارش
    return new Response(JSON.stringify({ message: 'Report fetched successfully', data: report }), {
      status: 200,
    })
  } catch (error) {
