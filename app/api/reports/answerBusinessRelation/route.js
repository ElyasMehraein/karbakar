...
export async function PUT(req) {
  try {
    ...
    if (parameter && businessRelation) {
      const receiverBusiness = JSON.parse(
        JSON.stringify(await BusinessModel.findOne({ _id: businessRelation.receiver }))
      )
      ...
      const providerBusiness = JSON.parse(
        JSON.stringify(await BusinessModel.findOne({ _id: businessRelation.provider }))
      )
