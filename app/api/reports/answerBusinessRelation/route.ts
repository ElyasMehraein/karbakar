...
export async function PUT(req: Request): Promise<NextResponse> {
    try {
        ...
        const businessRelation = await BusinessRelationModel.findById(businessRelationID) as BusinessRelation | null;
        if (parameter && businessRelation) {
            const receiverBusiness = await BusinessModel.findOne({ _id: businessRelation.receiver }) as Business | null;
            if (!receiverBusiness) {
                return NextResponse.json(
            ...
            const providerBusiness = await BusinessModel.findOne({ _id: businessRelation.provider }) as Business | null;
            if (!providerBusiness) {
                return NextResponse.json(
            ...
            const providerBusinessAgent = await UserModel.findOne({ code: providerBusiness.agentCode }) as User | null;
