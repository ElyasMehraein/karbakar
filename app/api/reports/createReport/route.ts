  ...
  try {
    ...
    if (title === 'jobOffer') {
      if (recepiant.businesses.length >= 3) {
        return NextResponse.json(
          {
            message: 'users can only be a member of a maximum of 3 businesses',
          },
          { status: 405 }
        );
      }
      if (recepiant.code === user.code) {
        return NextResponse.json(
          { message: "406 you can't hire yourself!" },
          { status: 406 }
        );
      }
      const isEmployeeHere = JSON.parse(JSON.stringify(Business)).workers.some(
        (worker: string) => {
      ...
      if (isEmployeeHere) {
        return NextResponse.json(
          { message: 'This user is currently a member of this business' },
          { status: 409 }
        );
      }

      const isJobOfferExist = await ReportModel.findOne({
        recepiant,
        business: Business,
        isAnswerNeed: true,
      });
      if (isJobOfferExist) {
        return NextResponse.json(
          { message: 'This jobOffer already exist' },
          { status: 410 }
        );
      }
      isAnswerNeed = true;
