...
export async function PUT(req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const { provider, receiver } = body as { provider: string; receiver: string };
        ...
        const business = await BusinessModel.findById(provider).lean() as Business;
        if (!business) {
        ...
        return NextResponse.json({ message: "relation created successfully", data: newRelation }, { status: 201 });
