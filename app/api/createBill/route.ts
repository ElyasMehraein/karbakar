...
export async function POST(req: NextRequest) {
    try {
        ...
        const Business: Business = JSON.parse(JSON.stringify(await BusinessModel.findOne({ _id: businessID }).populate("guild")));

        const customer: User = JSON.parse(JSON.stringify(await UserModel.findOne({ code: customerCode })));
        if (!customer) {
        ...
        for (const item of basket) {
            ...
            let basketProduct = await ProductModel.findOne({ productName }) as Product | null;
            if (basketProduct) {
                products.push({
