...
export async function POST(req) {
  try {
    ...
    const isVoterMember = union.members.some(
      (m) => m.member.toString() === voterId.toString()
    );
    const isVoteForMember = union.members.some(
      (m) => m.member.toString() === voteForId.toString()
    );

    if (!isVoterMember || !isVoteForMember) {
      return new Response(
        JSON.stringify({ message: "Voter or voteFor is not a union member" }),
        { status: 403 }
      );
    }
    ...
    if (voteType === "approve") {
      ...
      return new Response(
        JSON.stringify({ message: "Invalid voteType" }),
        { status: 400 }
      );
    }
    ...
    return new Response(
      JSON.stringify({ message: "Vote processed successfully" }),
      { status: 200 }
    );
  } catch (error) {
