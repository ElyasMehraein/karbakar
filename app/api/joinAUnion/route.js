...
export async function POST(req) {
  try {
    ...
    const validatedOfferBasket = await validateAndCreateProducts(offerBasket, guildIdForOffer)

    // گیلد نیاز برای سبد موردنیاز
    const validatedDemandBasket = await validateAndCreateProducts(demandBasket, demandGuildID)
    ...
    const oldMembers = union.members.filter(m => m.member.toString() !== newMemberId)
