...
const ShowMyLocation = dynamic(
  () => import('@/components/modules/ShowMyLocation'),
  { ssr: false }
);
...
export default function AllBusinesses(): JSX.Element {
  ...
  useEffect(() => {
    if (businesses && latitude) {
      setBusinessesOrderByDistance(
        orderByDistance(
          { latitude, longitude },
          businesses
            .filter((business) => business.latitude)
            .map((business) => ({
                        ...
                        <ListItem
                          ...
                          secondaryAction={
                            <ListItemText
                              sx={{ ml: 5 }}
                              align="right"
                              primary={business.businessBrand}
                          ...
                          <ListItemText
                            primary={(
                              getDistance(
                                { latitude, longitude },
                                {
                                  latitude: business.latitude?.$numberDecimal,
                                  longitude: business.longitude?.$numberDecimal,
                                }
                              ) / 1000
                            ).toFixed()}
                            secondary="km"
