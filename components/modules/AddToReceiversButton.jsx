...
export default function AddToReceiversButton({ relations, logedUser, business }) {
  ...
  const providerBusinessNames = relations
    ...
    .map(relation => {
      const matchingBusiness = logedUserBusinesses.find(
        businessItem => businessItem._id === relation.provider._id
      )
  ...
  const buttonText = providerBusinessNames[0]
    ? `${[...providerBusinessNames]} `
    : 'ارائه محصول به این کسب و کار'

  //checkBox
  const logedUserProviders = relations
    .filter(relation => {
    ...
    logedUserBusinesses.length > 0 && (
      <Container sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Box>
            ...
            <DialogContent>
              ...
              <List component="div" disablePadding>
                {logedUserBusinesses.map(business => (
                  <ListItem key={business._id} disablePadding>
                    <ListItemButton
                      key={business._id}
                      value={business._id}
                      sx={{ minWidth: '250px' }}
          ...
          <Button dir="ltr" onClick={handleOpen} fullWidth sx={{ m: 1 }} variant="contained">
            {buttonText}
          </Button>
          <CustomSnackbar
