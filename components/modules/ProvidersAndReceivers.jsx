...
export default function ProvidersAndReceivers({ filteredRelations, title }) {
  ...
  return (
    ...
    >
      <Accordion
        sx={{ width: '100%', bgcolor: blue[50], my: 1 }}
        onChange={() => setTopAvatarsVisible(prev => !prev)}
      >
        <AccordionSummary
                ...
                {filteredRelations.map(relation => (
                  <ItsAvatar
                    ...
                    userCodeOrBusinessBrand={
                      title === 'تامین کنندگان'
                        ? relation.provider.businessName
                        : relation.receiver.businessName
                    }
                    isAvatar={
                      title === 'تامین کنندگان'
                        ? relation.provider.isAvatar
                        : relation.receiver.isAvatar
                    }
                    alt="businessBrand"
                  />
                ))}
          ...
          {filteredRelations.map(relation => (
            <Box key={relation._id}>
              <ListItemButton
                onClick={() =>
                  router.push(
                    `/${title === 'تامین کنندگان' ? relation.provider.businessName : relation.receiver.businessName}`
                  )
                }
              >
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <ListItemAvatar>
                      <ListItemText
                        ...
                        secondary={
                          title === 'تامین کنندگان'
                            ? relation.provider.businessBrand
