...
export default function BasketSelection({ business, guild, parentBasketFunction }) {
      ...
      const fetchMonthlyCommitment = async () => {
        try {
          const response = await fetch(
            `/api/getBusinessMonthlyCommitment?businessId=${business._id}`
          )
  ...
  useEffect(() => {
    const matchedProduct = products.find(product => product.productName === selectedProductName)
    if (matchedProduct) {
  ...
  const isButtonDisable = !(selectedProductName && unitOfMeasurement && amount)
  ...
  return (
    <Container maxWidth="md" className="inMiddle" align="center">
      ...
      {basket.length > 0 && (
        <List
          dense
          sx={{ mb: 2, bgcolor: blue[50], p: 2, borderRadius: 2 }}
          ...
          {basket.map(productFrame => {
            return (
              ...
              >
                ...
                />
                <IconButton onClick={() => deleteFrame(productFrame.product.productName)}>
                  <DeleteIcon />
                </IconButton>
              </ListItem>
            )
