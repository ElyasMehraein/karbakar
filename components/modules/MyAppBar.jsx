...
export default function MyAppBar({ user, logedUserCode, business }) {
  ...
  const isAauthorizedToEdit = user
    ? logedUserCode === user.code
    : logedUserCode == business?.agentCode
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="static">
        <Toolbar>
          <Button size="large" edge="start" color="inherit" aria-label="menu" onClick={goToIndex}>
            <ArrowForwardIcon />
          ...
          {isAauthorizedToEdit && !pathname.endsWith('/edit') && isEditNeeded && (
            <Button color="inherit" onClick={goToEdit}>
              تنظیمات
              <EditIcon />
            </Button>
          )}
