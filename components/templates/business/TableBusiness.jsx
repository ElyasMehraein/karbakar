...
export default function TableBusiness({ business }) {
  const rows = business.deliveredProducts.map(product => {
    return {
      name: product.uniqueCustomer,
      code: product.productName,
      population: product.unitOfMeasurement,
      size: product.totalDelivered,
    }
  })
  ...
  return !rows[0] ? (
    <Typography align="center" color="error">
      این کسب و کار تا بحال هیچ محصولی به کسی تحویل نداده است{' '}
    </Typography>
  ) : (
    <Box
    ...
    >
      <Paper sx={{ p: 1, backgroundColor: grey[200], borderRadius: '30px' }}>
          ...
          <TableContainer sx={{ maxHeight: 500 }}>
            ...
            >
              ...
              >
                <TableRow>
                  {columns.map(column => (
                    <TableCell key={column.id} align={column.align} style={{ minWidth: 80 }}>
                      {column.label}
                    </TableCell>
                  ))}
                </TableRow>
