...
function Business({ relations, business, logedUser, bills }) {
  const providerRelations = relations?.filter(
    relation => !relation?.isAnswerNeed && relation?.receiver?._id === business?._id
  )

  const receiverRelations = relations?.filter(
    relation => !relation?.isAnswerNeed && relation?.provider?._id === business?._id
  )
  ...
  return (
    <>
      ...
      <ProvidersAndReceivers
        key={'providers' + business._id}
        filteredRelations={providerRelations}
        title={'تامین کنندگان'}
      />
      <ProvidersAndReceivers
        key={'receivers' + business._id}
        filteredRelations={receiverRelations}
