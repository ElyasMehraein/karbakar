    ...
    <Box>
      ...
      >
        ...
        {user?.businesses[0] && (
          <List sx={{ m: 0, p: 0 }}>
            <>
              <Divider sx={{ fontSize: 12 }} textAlign="center">
                کسب و کار اصلی
              </Divider>
            </>
            ...
            <React.Fragment key={primeBusiness._id}>
              <ListItemText
                align="right"
                secondary={'صنف:' + primeBusiness.guild.guildName}
                sx={{ mr: 2, mt: 0 }}
              />
              <ListItemButton
                sx={{ mt: -1, p: 0 }}
                onClick={() => router.push(`/${primeBusiness.businessName}`)}
              >
                <ListItem>
                  <ListItemIcon>
                    <ItsAvatar
                      isAvatar={primeBusiness.isAvatar}
                      userCodeOrBusinessBrand={primeBusiness.businessName}
                    />
                  </ListItemIcon>
                  <ListItemText
                    align="right"
                    primary={primeBusiness.businessName}
                    secondary={primeBusiness.businessBrand}
                    sx={{ m: 0 }}
                  />
                </ListItem>
              </ListItemButton>
            ...
            {user.businesses.length > 1 && (
              <>
                ...
                <Divider sx={{ fontSize: 12, mt: 2 }} textAlign="center">
                  کسب و کار های فرعی
                </Divider>
              </>
            )}
        ...
        <List>
          ...
          {user?.businesses[0] && <Resignation user={user} />}
          <ListItem disablePadding>
            <ListItemButton>
              <ListItemIcon>
                <BusinessIcon />
