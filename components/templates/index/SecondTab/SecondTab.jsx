...
export default function SecondTab({ primeBusiness }) {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          const totalWorkers = data.reduce(
            (acc, business) => acc + (business.workers?.length || 0),
            0
          )
  ...
  return (
    <Container maxWidth="md">
      <AccordionServise>{SecondTabText()}</AccordionServise>
      <Box
        className="inMiddle"
        sx={{
      ...
      >
        ...
        {businesses.length ? (
                ...
                >
                  ...
                  <AccordionDetails>
                    {business.demandsForGuilds
                      ...
                      .join(', ') || 'کاربر برای این درخواست توضیحی قرار نداده است'}
                  </AccordionDetails>
          ...
          !businesses.length &&
          totalWorkers === 0 && <Typography>کسب و کاری برای نمایش وجود ندارد </Typography>
        )}
      </Box>
    </Container>
  )
