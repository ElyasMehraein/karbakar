...
function hasUserVoted(union, voterId, voteForId) {
  ...
  return union.votes.some(
    v => v.voter.toString() === voterId.toString() && v.voteFor.toString() === voteForId.toString()
  )
}
...
export default function UnionsAccordionDetails({ union, user }) {
  ...
  const MemberInfo = ({ member }) => {
    return (
      ...
      >
        <ItsAvatar userCodeOrBusinessBrand={member.member.businessName} />
  ...
  const renderVoteColumn = member => {
    ...
    if (userBusinessId === member.member._id.toString()) {
      return (
        <Button size="small" variant="outlined" color="error" onClick={unionResignHandler}>
          انصراف
        </Button>
      )
    }
      ...
      {sortedMembers.map(member => {
        ...
        return (
          <Paper key={member._id} sx={{ p: 1, border: '1px solid #ddd', borderRadius: 2 }}>
            ...
            {/* پیشنهادها */}
            <Divider sx={{ my: 1 }} />
            <Typography sx={{ fontWeight: 'bold', fontSize: 13, mb: 1 }}>پیشنهادها</Typography>
            ...
            {/* نیازها */}
            <Divider sx={{ my: 1 }} />
            <Typography sx={{ fontWeight: 'bold', fontSize: 13, mb: 1 }}>نیازها</Typography>
            ...
            {!union.isActive && (
              <>
                <Divider sx={{ my: 1 }} />
                <Typography sx={{ fontWeight: 'bold', fontSize: 13, mb: 1 }}>رأی</Typography>
                {renderVoteColumn(member)}
              </>
            )}
          </Paper>
  ...
  return (
    <AccordionDetails sx={{ bgcolor: 'white', borderTop: `1px solid ${blue[100]}` }}>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>آیا درباره خروج از این اتحاد اطمینان دارید؟</DialogContentText>
        </DialogContent>
        <DialogActions>
